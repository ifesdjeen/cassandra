...
//              implicitly:  leaf().count = MAX_KEYS;
        ...
        private Insert updateRecursive(Insert ik, Object[] unode, Existing uub, BranchBuilder builder)
        {
            ...
            while (ik != null)
            {
                ...
                if (c < 0)
                {
                    ...
                    childBuilder.sourceNode = childUNode;
    ...
    static abstract class AbstractSeekingTransformer<I, O> extends AbstractUpdater implements AutoCloseable
    {
        ...
        Object[] apply()
        {
            ...
            for (int d = 0; d < update.leafDepth; ++d)
                level = level.ensureParent();
            level.sourceNode = unode;
        ...
        void finishTo(LeafOrBranchBuilder to)
        {
            LeafOrBranchBuilder finish = leaf();
            while (finish != to)
            {
                Invariants.require(finish.isEmpty() || finish.isSufficient());
                if (!finish.isEmpty())
                    finish(finish);

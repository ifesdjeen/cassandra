...
{
    static final long UNFILTERED_TO_UPDATE_PROGRESS = 100;
    ...
    public CompactionIterator(OperationType type,
                              ...
                              ActiveCompactionsTracker activeCompactions,
                              TopPartitionTracker.Collector topPartitionCollector,
                              // TODO (required): decouple compaction iterator from Accord
                              @Nonnull Supplier<IAccordService> accordService)
    {
        ...
        Transformation<UnfilteredRowIterator> purger = purger(controller.cfs, controller, this::updateBytesRead, nowInSec, accordService);
    ...
    protected Transformation<UnfilteredRowIterator> purger(ColumnFamilyStore cfs, AbstractCompactionController controller, Runnable progress, long nowInSec, Supplier<IAccordService> accordService)
    {
        if (isPaxos(cfs) && paxosStatePurging() != legacy)
            return new AbstractPurger.PaxosPurger(type, controller, nowInSec, progress);
        ...
        if (isAccordJournal(cfs))
            return new AbstractPurger.AccordJournalPurger(type, controller, nowInSec, progress, accordService);
        if (isAccordCommands(cfs))
            return new AbstractPurger.AccordCommandsPurger(type, controller, nowInSec, progress, accordService);
        if (isAccordTimestampsForKey(cfs))
            return new AbstractPurger.AccordTimestampsForKeyPurger(type, controller, nowInSec, progress, accordService);
        if (isAccordCommandsForKey(cfs))
            return new AbstractPurger.AccordCommandsForKeyPurger(type, controller, nowInSec, progress, AccordKeyspace.CommandsForKeysAccessor, accordService);

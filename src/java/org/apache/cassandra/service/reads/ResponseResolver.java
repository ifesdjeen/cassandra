...
public abstract class ResponseResolver<E extends Endpoints<E>, L extends ReplicaLayout<E, L>>
{
    ...
    public ResponseResolver(ReadCommand command, L replicaPlan, ReadRepair<E, L> readRepair, long queryStartNanoTime)
    {
        this.command = command;
        this.replicaPlan = replicaPlan;
        this.readRepair = readRepair;
        this.responses = new Accumulator<>(replicaPlan.natrualReplicas().size());
    ...
    public void preprocess(MessageIn<ReadResponse> message)
    {
        ...
        {
            logger.error("Encountered error while trying to preprocess the message {}: %s in command {}, replicas: {}", message, command, readRepair, replicaPlan.consistencyLevel(), replicaPlan.selectedReplicas());
            throw e;
        }
    }

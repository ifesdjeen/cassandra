...
public abstract class AbstractReadExecutor
{
    ...
    private final ReplicaLayout.ForToken replicaPlan;
    ...
    AbstractReadExecutor(ColumnFamilyStore cfs, ReadCommand command, ReplicaLayout.ForToken replicaPlan, long queryStartNanoTime)
    {
        ...
        this.handler = new ReadCallback<>(digestResolver, replicaPlan.blockFor(), command, replicaPlan, queryStartNanoTime);
        this.cfs = cfs;
    ...
    public static AbstractReadExecutor getReadExecutor(SinglePartitionReadCommand command, ConsistencyLevel consistencyLevel, long queryStartNanoTime) throws UnavailableException
    {
        ...
        // Endpoints for Token
        ReplicaLayout.ForToken replicaPlan = ReplicaLayout.forRead(keyspace, command.partitionKey().getToken(), consistencyLevel, retry);
        ...
        if (consistencyLevel.blockFor(keyspace) == replicaPlan.natrualReplicas().size())
        ...
        if (replicaPlan.natrualReplicas().size() == replicaPlan.selectedReplicas().size() || retry.equals(AlwaysSpeculativeRetryPolicy.INSTANCE))
    ...
    ReplicaLayout.ForToken replicaPlan()
    {
        return replicaPlan;
    }
    ...
    public static class NeverSpeculatingReadExecutor extends AbstractReadExecutor
    {
        ...
        public NeverSpeculatingReadExecutor(ColumnFamilyStore cfs, ReadCommand command, ReplicaLayout.ForToken replicaPlan, long queryStartNanoTime, boolean logFailedSpeculation)
        {
        ...
        {
            makeDataRequests(replicaPlan().selectedReplicas().subList(0, 1));
            if (replicaPlan().selectedReplicas().size() > 1)
                makeDigestRequests(replicaPlan().selectedReplicas().subList(1, replicaPlan().selectedReplicas().size()));
        }
    ...
    {
        private volatile ReplicaLayout.ForToken replicaPlan;
        ...
        public ReplicaLayout.ForToken replicaPlan()
        {
            return replicaPlan;
        }
        ...
        public void executeAsync()
        {
            EndpointsForToken initialReplicas = replicaPlan().selectedReplicas();
        ...
        public void maybeTryAdditionalReplicas()
        {
            if (shouldSpeculateAndMaybeWait())
            {
                ...
                speculated = true;
                replicaPlan = ReplicaLayout.forSpeculation(replicaPlan);
                cfs.metric.speculativeRetries.inc();
                // Could be waiting on the data, or on enough digests.
                Replica extraReplica = Iterables.getLast(replicaPlan().selectedReplicas());
    ...
    private static class AlwaysSpeculatingReadExecutor extends AbstractReadExecutor
    {
        ...
        public void executeAsync()
        {
            makeDataRequests(replicaPlan().selectedReplicas().subList(0, replicaPlan().selectedReplicas().size() > 1 ? 2 : 1));
            if (replicaPlan().selectedReplicas().size() > 2)
                makeDigestRequests(replicaPlan().selectedReplicas().subList(2, replicaPlan().selectedReplicas().size()));
            cfs.metric.speculativeRetries.inc();
        }

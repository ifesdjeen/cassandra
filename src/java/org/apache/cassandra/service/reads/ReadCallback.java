...
public class ReadCallback<E extends Endpoints<E>, L extends ReplicaLayout<E, L>> implements IAsyncCallbackWithFailure<ReadResponse>
{
    ...
    public ReadCallback(ResponseResolver resolver, int blockfor, ReadCommand command, L replicaPlan, long queryStartNanoTime)
    {
        ...
        assert !(command instanceof PartitionRangeReadCommand) || blockfor >= replicaPlan.selectedReplicas().size();
    ...
    public void awaitResults() throws ReadFailureException, ReadTimeoutException
    {
        ...
        boolean failed = failures > 0  && blockfor + failures > replicaPlan.selectedReplicas().size();
        if (signaled && !failed)
            return;
    ...
    {
        replicaPlan.consistencyLevel().assureSufficientLiveNodes(replicaPlan.keyspace(), replicaPlan.selectedReplicas());
    }
    ...
    public void onFailure(InetAddressAndPort from, RequestFailureReason failureReason)
    {
        ...
        if (blockfor + n > replicaPlan.selectedReplicas().size())
            condition.signalAll();
    }
}

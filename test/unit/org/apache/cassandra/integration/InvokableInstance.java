...
public class InvokableInstance
{
    ...
    final Method deserializeOnInstance;

    public InvokableInstance(File root, InstanceConfig config, ClassLoader classLoader)
    {
        this.root = root;
        this.config = config;
    ...
    public <T> SerializableConsumer<T> consumesOnInstance(SerializableConsumer<T> consumer) { return (SerializableConsumer<T>) transferOneObject(consumer); }

    public interface SerializableBiConsumer<T1, T2> extends BiConsumer<T1, T2>, Serializable {}
    public <T1, T2> SerializableBiConsumer<T1, T2> consumesOnInstance(SerializableBiConsumer<T1, T2> consumer) { return (SerializableBiConsumer<T1, T2>) transferOneObject(consumer); }
    ...
    public static class InstanceClassLoader extends URLClassLoader
    {
        private final ClassLoader commonClassLoader;
        private final Predicate<String> shared;

        public InstanceClassLoader(URL[] urls, Predicate<String> shared, ClassLoader commonClassLoader)
        {
            super(urls, null);
